...
- Set `AGENT_WORKDIR` to the repo root (e.g. `~/Documents/Iamnotjarvis`) so `soul.md` and memory files live alongside the project. Friday can edit its own `friday/` source via file tools when soul or user requests it; **restart** `python -m friday` after Python source changes, or enable `FRIDAY_SELF_RESTART_ENABLED=true` and use the `restart_friday` tool.
