...
TOOL_DEFINITIONS: List[Dict[str, Any]] = [
    ...
    },
    {
        ...
        "function": {
            "name": "analyze_image",
            "description": (
                "Analyze an image file (png/jpg/jpeg/gif) on disk with a vision model. "
                "Use for describing, transcribing, or answering questions about images, screenshots, diagrams, "
                "or scanned documents. Returns the model's analysis. Falls back to OpenRouter vision if the "
                "Azure deployment is not vision-capable."
            ),
            "parameters": {
                "type": "object",
                "properties": {
                    "path": {"type": "string", "description": "Path to the image file (relative to workdir or absolute)"},
                    "prompt": {
                        "type": "string",
    ...
    },
    {
        ...
        "function": {
            "name": "read_pdf",
            "description": (
                "Extract text from a PDF file on disk and optionally analyze its contents with the LLM. "
                "Use for summarizing, searching, or answering questions about PDF documents. "
                "Returns extracted text (truncated) and, when analyze=true, the model's analysis."
            ),
            "parameters": {
                "type": "object",
                "properties": {
                    "path": {"type": "string", "description": "Path to the PDF file (relative to workdir or absolute)"},
                    "analyze": {
                        "type": "boolean",
                        "description": "Also ask the LLM to analyze/summarize the extracted text; default false",
                    },
                    "prompt": {
                        "type": "string",
                        "description": "Analysis question/prompt used when analyze=true; default 'Summarize this document.'",
                    },
                    "max_chars": {
                        "type": "integer",
                        "description": "Max characters of extracted text to return/analyze; default 8000",
                    },
                },
                "required": ["path"],
            },
    ...
    },
    {
        ...
        "function": {
            ...
            "description": (
                "Search the web using SerpAPI (structured results) when SERPAPI_KEY is set. "
                "Use as an alternative to the built-in DuckDuckGo web_search for richer, structured snippets. "
                "Returns formatted results or an error if SerpAPI is unavailable."
            ),
            "parameters": {
                "type": "object",
                "properties": {
...
def _friday_paths() -> Dict[str, Optional[str]]:
    ...
    # The installed package lives at <repo>/friday/friday; the project dir
    # (setup.py/pyproject.toml) is its parent, and the repo root is one level up.
    project_dir = package_dir.parent
    repo_root: Optional[Path] = None
    if (project_dir / "setup.py").is_file() or (project_dir / "pyproject.toml").is_file():
...
async def execute_tool(ctx: ToolContext, name: str, arguments: Dict[str, Any]) -> str:
    ...
    if name == "analyze_image":
        from friday.llm.multimodal import encode_file_base64, openrouter_vision

        path = _resolve_path(ctx.workdir, str(arguments.get("path", "")))
        prompt = str(arguments.get("prompt") or "Describe this image in detail.").strip()
        if not path.is_file():
            return json.dumps({"error": "file not found", "path": str(path)})
        try:
            image_b64, mime = encode_file_base64(path)
        except OSError as exc:
            return json.dumps({"error": str(exc)})
        # Primary: Azure OpenAI vision deployment.
        try:
            content, usage = ctx.llm.vision(prompt, image_b64, mime)
            if content:
                if ctx.usage_store and ctx.settings.token_usage_enabled:
                    ctx.usage_store.record("analyze_image", usage)
                return json.dumps(
                    {"path": str(path), "prompt": prompt, "analysis": content, "source": "azure"},
                    ensure_ascii=False,
                )
        except Exception as exc:
            logger.warning(f"Azure vision failed for {path}: {exc}")
        # Fallback: OpenRouter vision (chat/ capability).
        content, used_model = openrouter_vision(prompt, image_b64, mime)
        if content:
            return json.dumps(
                {"path": str(path), "prompt": prompt, "analysis": content, "source": "openrouter", "model": used_model},
                ensure_ascii=False,
            )
        return json.dumps(
            {
                "error": "vision analysis unavailable",
                "hint": "Azure deployment is not vision-capable and OPENROUTER_API_KEY is not set.",
                "path": str(path),
            }
        )

    if name == "read_pdf":
        from friday.llm.multimodal import extract_text_from_pdf, openrouter_chat

        path = _resolve_path(ctx.workdir, str(arguments.get("path", "")))
        analyze = bool(arguments.get("analyze", False))
        prompt = str(arguments.get("prompt") or "Summarize this document.").strip()
        max_chars = _as_int(arguments.get("max_chars"), 8000, maximum=32000)
        if not path.is_file():
            return json.dumps({"error": "file not found", "path": str(path)})
        try:
            ...
            return json.dumps({"error": str(exc)})
        text = extract_text_from_pdf(pdf_bytes, max_chars=max_chars)
        if text.startswith("Error:") or text.startswith("Warning:"):
            return json.dumps({"path": str(path), "error": text})
        result: Dict[str, Any] = {"path": str(path), "text": text, "chars": len(text)}
        if analyze:
            messages = [
                {"role": "system", "content": "You are an expert document analyst. Answer concisely using the provided document text."},
                {"role": "user", "content": f"{prompt}\n\nDocument ({path.name}):\n{text}"},
            ]
            try:
                analysis, usage = ctx.llm.chat(messages)
                if analysis:
                    if ctx.usage_store and ctx.settings.token_usage_enabled:
                        ctx.usage_store.record("read_pdf", usage)
                    result["analysis"] = analysis
                    result["source"] = "azure"
            except Exception as exc:
                logger.warning(f"Azure PDF analysis failed for {path}: {exc}")
            if "analysis" not in result:
                analysis, used_model = openrouter_chat(messages)
                if analysis:
                    result["analysis"] = analysis
                    result["source"] = "openrouter"
                    result["model"] = used_model
                else:
                    result["analysis_error"] = "PDF analysis unavailable (no vision/LLM fallback configured)"
        return json.dumps(result, ensure_ascii=False)

    if name == "web_search_serpapi":
        from friday.llm.multimodal import serpapi_search

        query = str(arguments.get("query", "")).strip()
        if not query:
            return json.dumps({"error": "empty query"})
        max_results = _as_int(arguments.get("max_results"), 5, maximum=20)
        return json.dumps(serpapi_search(query, max_results=max_results), ensure_ascii=False)
