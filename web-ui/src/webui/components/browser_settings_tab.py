...
def create_browser_settings_tab(webui_manager: WebuiManager):
    ...
    with gr.Group():
        with gr.Row():
            browser_binary_path = gr.Textbox(
                ...
                lines=1,
                value=sv(webui_manager, "browser_settings.browser_binary_path", ""),
                interactive=True,
            ...
            browser_user_data_dir = gr.Textbox(
                ...
                lines=1,
                value=sv(webui_manager, "browser_settings.browser_user_data_dir", ""),
                interactive=True,
    ...
    with gr.Group():
        with gr.Row():
            use_own_browser = gr.Checkbox(
                ...
                value=sv(
                    webui_manager,
                    "browser_settings.use_own_browser",
                    bool(strtobool(os.getenv("USE_OWN_BROWSER", "false"))),
                ),
            ...
            keep_browser_open = gr.Checkbox(
                ...
                value=sv(
                    webui_manager,
                    "browser_settings.keep_browser_open",
                    bool(strtobool(os.getenv("KEEP_BROWSER_OPEN", "true"))),
                ),
            ...
            headless = gr.Checkbox(
                label="Headless Mode",
                value=sv(webui_manager, "browser_settings.headless", False),
                info="Run browser without GUI",
                interactive=True
            )
            disable_security = gr.Checkbox(
                label="Disable Security",
                value=sv(webui_manager, "browser_settings.disable_security", False),
    ...
    with gr.Group():
        with gr.Row():
            window_w = gr.Number(
                label="Window Width",
                value=sv(webui_manager, "browser_settings.window_w", 1280),
                info="Browser window width",
                interactive=True
            )
            window_h = gr.Number(
                label="Window Height",
                value=sv(webui_manager, "browser_settings.window_h", 1100),
    ...
    with gr.Group():
        with gr.Row():
            cdp_url = gr.Textbox(
                label="CDP URL",
                value=sv(webui_manager, "browser_settings.cdp_url", os.getenv("BROWSER_CDP", None)),
                info="CDP URL for browser remote debugging",
                interactive=True,
            )
            wss_url = gr.Textbox(
                label="WSS URL",
                value=sv(webui_manager, "browser_settings.wss_url", ""),
    ...
    with gr.Group():
        with gr.Row():
            save_recording_path = gr.Textbox(
                ...
                value=sv(webui_manager, "browser_settings.save_recording_path", ""),
            ...
            save_trace_path = gr.Textbox(
                label="Trace Path",
                value=sv(webui_manager, "browser_settings.save_trace_path", ""),
        ...
        with gr.Row():
            save_agent_history_path = gr.Textbox(
                ...
                value=sv(webui_manager, "browser_settings.save_agent_history_path", "./tmp/agent_history"),
                info="Specify the directory where agent history should be saved.",
                interactive=True,
            )
            save_download_path = gr.Textbox(
                label="Save Directory for browser downloads",
                value=sv(webui_manager, "browser_settings.save_download_path", "./tmp/downloads"),
