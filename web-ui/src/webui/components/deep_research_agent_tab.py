...
from src.webui.setting_utils import coerce_int, init_value as sv
...
async def run_deep_research(webui_manager: WebuiManager, components: Dict[Component, Any]) -> AsyncGenerator[
    ...
    try:
        ...
        browser_config_dict = {
            ...
            "window_width": coerce_int(get_setting("browser_settings", "window_w", 1280), 1280),
            "window_height": coerce_int(get_setting("browser_settings", "window_h", 1100), 1100),
    ...
    with gr.Group():
        with gr.Row():
            ...
            mcp_server_config = gr.Textbox(
                label="MCP server",
                lines=6,
                value=sv(webui_manager, "deep_research_agent.mcp_server_config", ""),
                interactive=True,
                visible=False,
            )
    ...
    with gr.Group():
        ...
        with gr.Row():
            resume_task_id = gr.Textbox(
                ...
                value=sv(webui_manager, "deep_research_agent.resume_task_id", ""),
                interactive=True,
            )
            parallel_num = gr.Number(
                label="Parallel Agent Num",
                value=sv(webui_manager, "deep_research_agent.parallel_num", 1),
                precision=0,
                interactive=True,
            )
            max_query = gr.Textbox(
                label="Research Save Dir",
                value=sv(webui_manager, "deep_research_agent.max_query", "./tmp/deep_research"),
                interactive=True,
            )
    with gr.Row():
