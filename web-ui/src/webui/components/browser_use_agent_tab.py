...
from src.agent.goal_driven import build_goal_driven_agent_config
from src.browser.custom_browser import CustomBrowser
from src.controller.custom_controller import CustomController
from src.utils import llm_provider
from src.utils.prompt_file import load_default_prompt, save_default_prompt
from src.webui.setting_utils import init_value as sv
from src.webui.prompt_defaults import (
    RUN_AGENT_TAB_BLURB,
    prompt_textbox_lines,
    resolve_extend_system_prompt,
)
from src.agent.session_context import JobSessionContext, session_enabled
from src.agent.user_learning import user_learning_enabled
from src.agent.user_preferences import UserPreferenceStore
...
async def _handle_new_step(
    ...
    if webui_manager.bu_job_session and output and output.current_state:
        cs = output.current_state
        url = getattr(state, "url", "") or ""
        title = getattr(state, "title", "") or ""
        webui_manager.bu_job_session.update_from_step(
            url=url,
            title=title,
            memory=cs.memory or "",
        ...
        webui_manager.bu_job_session.save()
...
def _handle_done(webui_manager: WebuiManager, history: AgentHistoryList):
    ...
    if webui_manager.bu_job_session and final_result:
        webui_manager.bu_job_session.update_from_step(
            memory=str(final_result)[:500],
...
async def _ask_assistant_callback(
    ...
    if response:
        guidance = f'User guidance when asked for help: {str(response).strip()[:400]}'
        if getattr(webui_manager, "bu_job_session", None):
            webui_manager.bu_job_session.record_user_learning(
                guidance,
                learning_type="assistant",
            )
            webui_manager.bu_job_session.save()
        if getattr(webui_manager, "bu_user_preferences", None):
            webui_manager.bu_user_preferences.record_raw(guidance)
            webui_manager.bu_user_preferences.save()
...
async def run_agent_task(
    ...
    remember_session = get_setting("remember_session_context", session_enabled())
    learn_from_user = get_setting("learn_from_user_overrides", user_learning_enabled())
    if remember_session or learn_from_user:
        if webui_manager.bu_job_session is None:
            webui_manager.bu_job_session = JobSessionContext.load()
        if remember_session:
            webui_manager.bu_job_session.record_task_start(
                components.get(user_input_comp, "") or ""
            )
            webui_manager.bu_job_session.save()
    else:
        webui_manager.bu_job_session = None

    if learn_from_user:
        if webui_manager.bu_user_preferences is None:
            webui_manager.bu_user_preferences = UserPreferenceStore.load()
    else:
        webui_manager.bu_user_preferences = None
    ...
    try:
        save_default_prompt(task)
    except Exception as exc:
        gr.Warning(f"Task will run, but prompt.txt could not be saved: {exc}")
    ...
    # --- Agent Settings ---
    goal_driven_autonomy = get_setting("goal_driven_autonomy", True)
    job_application_mode = get_setting("job_application_mode", True)
    override_system_prompt = get_setting("override_system_prompt") or None
    extend_system_prompt = resolve_extend_system_prompt(
        get_setting("extend_system_prompt") or None
    )
    planner_interval = int(get_setting("planner_interval", 3))
    use_main_llm_as_planner = get_setting("use_main_llm_as_planner", True)
    ...
    enable_ocr = get_setting("enable_ocr", True)
    enable_html_inspect = get_setting("enable_html_inspect", True)
    ...
    gd_config = build_goal_driven_agent_config(
        goal_driven_enabled=bool(goal_driven_autonomy),
        job_application_mode=bool(job_application_mode),
        extend_system_prompt=extend_system_prompt,
        planner_llm=planner_llm,
        main_llm=main_llm,
        use_main_llm_as_planner=bool(use_main_llm_as_planner),
        planner_interval=planner_interval,
    )
    extend_system_prompt = gd_config.extend_system_message
    planner_llm = gd_config.planner_llm
    extend_planner_system_message = gd_config.extend_planner_system_message
    ...
    try:
        ...
        if learn_from_user and isinstance(
            webui_manager.bu_browser_context, CustomBrowserContext
        ):
            await webui_manager.bu_browser_context.ensure_user_learning()
        ...
        if not webui_manager.bu_agent:
            ...
            webui_manager.bu_agent = BrowserUseAgent(
                ...
                planner_llm=planner_llm,
                planner_interval=gd_config.planner_interval,
                use_vision_for_planner=planner_use_vision if planner_llm else False,
                goal_driven_autonomy=bool(goal_driven_autonomy),
                job_application_mode=bool(job_application_mode),
                enable_ocr=bool(enable_ocr),
                enable_html_inspect=bool(enable_html_inspect),
                enable_memory=gd_config.enable_memory,
                job_session_context=webui_manager.bu_job_session,
                learn_from_user_overrides=bool(learn_from_user),
                user_preference_store=webui_manager.bu_user_preferences,
                source="webui",
            )
            ...
            webui_manager.bu_agent._job_session = webui_manager.bu_job_session
            webui_manager.bu_agent._learn_from_user_overrides = bool(learn_from_user)
            webui_manager.bu_agent._user_preferences = webui_manager.bu_user_preferences
        ...
        try:
            ...
            if hasattr(webui_manager.bu_agent, "finalize_user_preferences"):
                await webui_manager.bu_agent.finalize_user_preferences()
            ...
            final_update.update(
                {
                    user_input_comp: gr.update(
                        ...
                        interactive=True,
                        placeholder="Task prompt (prompt.txt) — saved when you run.",
                    ),
...
async def handle_clear(webui_manager: WebuiManager):
    ...
    if session_enabled():
        webui_manager.bu_job_session = JobSessionContext()
        JobSessionContext.clear_persisted()
    if getattr(webui_manager, "bu_user_preferences", None) is not None or user_learning_enabled():
        webui_manager.bu_user_preferences = UserPreferenceStore()
        UserPreferenceStore.clear_persisted()
    ...
    return {
        ...
        webui_manager.get_component_by_id("browser_use_agent.user_input"): gr.update(
            value=load_default_prompt(),
            placeholder="Edit task prompt (saved to prompt.txt on run)...",
        ),
        webui_manager.get_component_by_id(
            "browser_use_agent.agent_history_file"
...
def create_browser_use_agent_tab(webui_manager: WebuiManager):
    ...
    tab_components = {}
    with gr.Column():
        gr.Markdown(RUN_AGENT_TAB_BLURB)
        chatbot = gr.Chatbot(
        ...
        _default_prompt = load_default_prompt()
        _task_prompt = sv(
            webui_manager,
        ...
        user_input = gr.Textbox(
            label="Task prompt (web-ui/prompt.txt)",
            placeholder="Login, profile, and job rules — saved to prompt.txt when you run.",
            value=_task_prompt,
            lines=prompt_textbox_lines(_task_prompt),
            interactive=True,
            elem_id="user_input",
        )
        with gr.Row():
            reload_prompt_button = gr.Button(
                "↻ Reload prompt.txt", variant="secondary", scale=1
            )
    ...
    tab_components.update(
        dict(
            ...
            reload_prompt_button=reload_prompt_button,
    ...
    def reload_prompt_from_file():
        text = load_default_prompt()
        return gr.update(value=text, lines=prompt_textbox_lines(text))

    reload_prompt_button.click(
        fn=reload_prompt_from_file,
        inputs=None,
        outputs=[user_input],
    )
