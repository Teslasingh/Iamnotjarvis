...
from src.webui.settings_persistence import (
    collect_settings,
    load_saved_settings,
...
class WebuiManager:
    def __init__(self, settings_save_dir: str = "./tmp/webui_settings"):
        ...
        self.saved_settings: dict = load_saved_settings(self)
    ...
    def init_browser_use_agent(self) -> None:
        ...
        self.bu_job_session: Optional[JobSessionContext] = (
            JobSessionContext.load() if session_enabled() else None
        )
        self.bu_user_preferences: Optional[UserPreferenceStore] = None
    ...
    def save_config(self, components: Dict["Component", str]) -> None:
        """
        Save config snapshot and update last-used settings for auto-restore.
        """
        cur_settings = collect_settings(self, components)

        config_name = datetime.now().strftime("%Y%m%d-%H%M%S")
        snapshot_path = os.path.join(self.settings_save_dir, f"{config_name}.json")
        with open(snapshot_path, "w", encoding="utf-8") as fw:
            json.dump(cur_settings, fw, indent=2, ensure_ascii=False)

        write_saved_settings(self, cur_settings)
    ...
    def load_config(self, config_path: str):
        ...
        if isinstance(ui_settings, dict):
            self.saved_settings = ui_settings
            write_saved_settings(self, ui_settings)
